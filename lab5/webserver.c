...
{
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <read_msg_threads> <read_file_threads> <send_msg_threads>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    int read_msg_threads = atoi(argv[1]);
    int read_file_threads = atoi(argv[2]);
    int send_msg_threads = atoi(argv[3]);

    if (read_msg_threads <= 0 || read_file_threads <= 0 || send_msg_threads <= 0) {
        fprintf(stderr, "All thread counts must be positive integers\n");
        exit(EXIT_FAILURE);
    }

    // Initialize threadpools with specified number of threads
    read_msg_threadpool = thpool_init(read_msg_threads);
    read_file_threadpool = thpool_init(read_file_threads);
    send_msg_threadpool = thpool_init(send_msg_threads);

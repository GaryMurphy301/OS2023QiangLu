...
void *monitor_thpool(void *arg)
{
	...
	thpool_monitor monitor;

	while (1)
	{
		sleep(10); // Frequency of monitoring in seconds
		get_thpool_status(thpool, &monitor);

		printf("Threads alive: %d, Threads working: %d, Job queue length: %d\n",
			   monitor.num_threads_alive,
			   monitor.num_threads_working,
			   monitor.job_queue_length);
	}
	return NULL;
}

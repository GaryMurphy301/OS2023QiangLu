...
struct threadpool *initThreadPool(int num_threads)
{
    ...
    {
        pool->threads[i] = (struct thread *)malloc(sizeof(struct thread)); // Allocate memory for each thread
        if (pool->threads[i] == NULL)
        {
            error("creat thread(): Could not allocate memory for thread\n");
            return -1;
        }
        pool->threads[i]->id = i;                                                                        // Set the thread ID
        pool->threads[i]->pool = pool;                                                                   // Set the thread's pool
        pthread_create(&(pool->threads[i]->pthread), NULL, (void *)thread_do, (void *)pool->threads[i]); // Create the thread
        pthread_detach(pool->threads[i]->pthread);                                                       // Detach the thread
    }
